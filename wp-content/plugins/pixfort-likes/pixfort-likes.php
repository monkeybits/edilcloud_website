...
define( 'PIXFORT_LIKES_VERSION', '1.0' );
...
class PixFortLikes {
	...
	function pixfort_likes_textdomain() {
		...
		load_plugin_textdomain( 'pixfortlikes', false, $pixfort_likes_lang_dir );
	}
	...
	function admin_init(){
		...
		add_settings_field( 'like_icon', __('Like icon', 'pixfortlikes'), array(&$this, 'setting_like_icon'), 'pixfort-likes', 'pixfort-likes');
		add_settings_field( 'ajax_likes', __('AJAX Like Counts', 'pixfortlikes'), array(&$this, 'setting_ajax_likes'), 'pixfort-likes', 'pixfort-likes');
		add_settings_field( 'instructions', __( 'Shortcode and Template Tag', 'pixfortlikes' ), array(&$this, 'setting_instructions'), 'pixfort-likes', 'pixfort-likes' );
	}

	function admin_menu(){
		$icon_url = plugins_url( '/images/pixfort-likes-icon.svg', __FILE__ );
		$page_hook = add_menu_page( __( 'pixfort Likes Settings', 'pixfortlikes'), 'pixfort Likes', 'update_core', 'pixfort-likes', array(&$this, 'settings_page'), $icon_url );
		add_submenu_page( 'pixfort-likes', __( 'Settings', 'pixfortlikes' ), __( 'pixfort Likes Settings', 'pixfortlikes' ), 'update_core', 'pixfort-likes', array(&$this, 'settings_page') );
	...
	function settings_page(){
		...
		<div class="wrap">
			...
			<h2><?php _e('pixfort Likes Settings', 'pixfortlikes'); ?></h2>
			<?php if( isset($_GET['settings-updated']) && $_GET['settings-updated'] ){ ?>
			<div id="setting-error-settings_updated" class="updated settings-error">
				<p><strong><?php _e( 'Settings saved.', 'pixfortlikes' ); ?></strong></p>
			</div>
			<?php } ?>
			<form action="options.php" method="post">
				<?php settings_fields( 'pixfort-likes' ); ?>
				<?php do_settings_sections( 'pixfort-likes' ); ?>
				<p class="submit"><input type="submit" class="button-primary" value="<?php _e( 'Save Changes', 'pixfortlikes' ); ?>" /></p>
			</form>
		</div>
		<?php
	}
	...
	function section_intro(){
	    ?>
		<p><?php _e('PixFort Likes allows you to display like icons throughout your site.', 'pixfortlikes'); ?></p>
		<?php
	...
	function setting_show_on(){
		...
		'. __('Posts', 'pixfortlikes') .'</label><br />
		...
		'. __('Pages', 'pixfortlikes') .'</label><br />
		...
		'. __('Blog Index Page, Archive Pages, and Search Results', 'pixfortlikes') .'</label><br />';
	}
	...
	function setting_exclude_from(){
		...
		<p class="description">'. __('Comma separated list of post/page ID\'s (e.g. 4,7,87)', 'pixfortlikes') . '</p>';
	}
	...
	function setting_disable_css(){
		...
		<label><input type="checkbox" name="pixfort_likes_settings[disable_css]" value="1"'. (($options['disable_css']) ? ' checked="checked"' : '') .' />' . __('I want to use my own CSS styles', 'pixfortlikes') . '</label>';
		...
		if(strtolower($theme_name) == 'shutterbug'){
    		echo '<br /><span class="description" style="color:red">'. __('We recommend you check this option when using the Shutterbug theme to avoid conflicts', 'pixfortlikes') .'</span>';
		}
	}
	...
	function setting_ajax_likes(){
		...
		' . __('AJAX Like Counts on page load', 'pixfortlikes') . '</label><br />
		<span class="description">'. __('If you are using a cacheing plugin, you may want to dynamically load the like counts via AJAX.', 'pixfortlikes') .'</span>';
	}
	...
	function setting_zero_postfix(){
		...
		<span class="description">'. __('The text after the count when no one has liked a post/page. Leave blank for no text after the count.', 'pixfortlikes') .'</span>';
	}
	...
	function setting_one_postfix()
	{
		...
		<span class="description">'. __('The text after the count when one person has liked a post/page. Leave blank for no text after the count.', 'pixfortlikes') .'</span>';
	}
	...
	function setting_more_postfix()
	{
		...
		<span class="description">'. __('The text after the count when more than one person has liked a post/page. Leave blank for no text after the count.', 'pixfortlikes') .'</span>';
	}

	function setting_instructions()
	{
		echo '<p>'. __('To use PixFort Likes in your posts and pages you can use the shortcode:', 'pixfortlikes') .'</p>
		<p><code>[pixfort_likes]</code></p>
		<p>'. __('To use pixfort Likes manually in your theme template use the following PHP code:', 'pixfortlikes') .'</p>
	...
	function do_likes($style='default'){
		...
		$output = $this->like_this($post->ID, $options['zero_postfix'], $options['one_postfix'], $options['more_postfix']);
  		...
  		$class = 'pixfort-likes';
  		$title = __('Like this', 'pixfortlikes');
		if( isset($_COOKIE['pixfort_likes_'. $post->ID]) ){
			$class = 'pixfort-likes active';
			$title = __('You already like this', 'pixfortlikes');
		}
        if($style=='box'){
