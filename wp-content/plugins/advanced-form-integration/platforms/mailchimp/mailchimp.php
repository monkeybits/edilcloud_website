...
function adfoin_mailchimp_settings_view( $current_tab ) {
    ...
    <form name="mailchimp_save_form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"
        ...
        <table class="form-table">
            <tr valign="top">
                <th scope="row"> <?php _e( 'Mailchimp API Key', 'advanced-form-integration' ); ?></th>
                <td>
                    <input type="text" name="adfoin_mailchimp_api_key"
                           ...
                           class="regular-text"/>
                    <p class="description" id="code-description"><?php _e( 'Please go to Account > Extras > API Keys', 'advanced-form-integration' ); ?></a></p>
                </td>
            </tr>
        </table>
        <?php submit_button(); ?>
...
add_action( 'admin_post_adfoin_save_mailchimp_api_key', 'adfoin_save_mailchimp_api_key', 10, 0 );
...
function adfoin_mailchimp_send_data( $record, $posted_data ) {
    ...
    if( $task == "unsubscribe" ) {
        ...
        if( !is_wp_error( $member ) ) {
            ...
            adfoin_add_to_log( $return, $unsub_url, $unsub_args, $record );
