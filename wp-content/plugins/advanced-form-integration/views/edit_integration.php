...
$nonce        = wp_create_nonce( 'adfoin_integration' );
...
<div class="wrap">
    ...
    <h1> <?php esc_attr_e( 'Update Integration: ', 'advanced-form-integration' ); echo $integration_title; ?></h1>
...
<script type="text/template" id="editable-field-template">
    <tr class="alternate" v-if="inArray(action.task, field.task)">
        ...
        <td>
            <input v-if="field.type == 'text'" type="text" ref="fieldValue" class="regular-text" v-model="fielddata[field.value]"  v-bind:required="field.required">
            <textarea rows="5" cols="46" v-if="field.type == 'textarea'" type="text" ref="fieldValue" v-model="fielddata[field.value]"  v-bind:required="field.required"></textarea>
    ...
    <tr valign="top" class="alternate" v-if="action.cl.active == 'yes'">
        ...
        <td>
            ...
            <select v-model="condition.operator">
                <?php
                $oprators = adfoin_get_cl_conditions();

                foreach ( $oprators as $key => $value ) {
                    echo "<option value='" . $key . "'> " . $value . " </option>";
                }
                ?>
            </select>
