			...
			vc.frame_window.pix_cb_fn(function(){
				let elem = el.closest('.pix_tabs_container');
					...
					contents.each(function(i, tab){
						...
						html += '<div class="nav-item"><a class="nav-link pix-tabs-btn pix-px-25 text-24 '+bold+' '+italic+' '+secondary+' py-2 mb-2" data-id="'+ id +'" id="pix-tab-btn-'+id+'" data-toggle="pill" href="#pix-tab-'+id+'" role="tab" aria-controls="pix-tab-'+id+'" aria-selected="true"><strong>'+ icon_html + title +'</strong></a></div>';
						if(first){
							$(tab).addClass('active');
...
(function ( $ ) {
    ...
    window.InlineShortcodeView_pix_vertical_tabs = window.InlineShortcodeViewContainer.extend({
    	...
    	pix_update: function () {
			...
			// that.$el.find('.pix_tabs_container').each(function(i, elem){
			// 	console.log("%%%%%%%");
		    //     var contents = $(elem).find('.tab-pane');
		    //     var html = '';
		    //     contents.each(function(i, tab){
		    //         // console.log(tab);
		    //         // $(elem).find('a:first-child').tab('show');
		    //         var id = $(tab).data('id');
		    //         var icon = $(tab).data('icon');
		    //         var title = $(tab).data('title');
		    //         var bold = $(tab).data('bold');
		    ...
		    //             icon_html = '<i class="'+icon+' mr-2"></i> ';
		    //         }
		    //         html += '<div class="nav-item">asd<a class="nav-link pix-tabs-btn text-24 '+bold+' '+italic+' '+secondary+' py-2 mb-2" data-id="'+ id +'" id="pix-tab-btn-'+id+'" data-toggle="pill" href="#pix-tab-'+id+'" role="tab" aria-controls="pix-tab-'+id+'" aria-selected="true">'+ icon_html + title +'</a></div>';
		    //         $(tab).removeClass('active');
		    //     });
			// 	$(elem).find('.pix_tabs_btns').html('');
		    //     $(elem).find('.pix_tabs_btns').append(html);
		    //     $(elem).find('.pix_tabs_btns .nav-item:first-child a').tab('show');
			//
		    // });
