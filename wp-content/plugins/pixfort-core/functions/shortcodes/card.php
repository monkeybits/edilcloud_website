        ...
        if($layout=='small'){
            $output .= '<a '.$linkTarget.' href="'.$link.'" class=" fly">';
                ...
                if( !empty($image) ) {
                    ...
                    $output .= '<img src="'.PIX_IMG_PLACEHOLDER .'" data-srcset="'.$imgSrcset.'" data-src="'.$imgSrc.'" class="pix-lazy" loading="lazy" alt="'. $title .'">';
                }
        ...
        if($layout=='big_padding'){
            $output .= '<div class="pix-content-box pix-card-element bg-white2 p-4 rounded-xl '.$classes.'" '.$anim_attrs.'>';
                if( !empty($image) ) {
                    ...
                    $output .= '<img src="'.PIX_IMG_PLACEHOLDER .'" data-srcset="'.$imgSrcset.'" data-src="'.$imgSrc.'" class="pix-lazy img-fluid '.$rounded_img.'" loading="lazy" alt="'. $title .'">';
                }
                  ...
                  $output .= '<div class="card-footer text-right">';
                    $output .= '<a '.$linkTarget.' href="'.$link.'" class="d-flex align-items-center justify-content-end '.$link_classes.'">'.$link_text.' <i class="pixicon-angle-right ml-2"></i></a>';
                  $output .= '</div>';
                $output .= '</div>';
        }
        if($layout=='big'){
            $output .= '<div class="card pix-content-box pix-card-element bg-white2 rounded-xl '.$classes.'" '.$anim_attrs.'>';
                if( !empty($image) ) {
                  ...
                  $output .= '</div>';
                  $output .= '<div class="card-footer text-right">';
                    $output .= '<a '.$linkTarget.' href="'.$link.'" class="d-flex align-items-center justify-content-end '.$link_classes.'"><span>'.$link_text.'</span><i class="pixicon-angle-right ml-2"></i></a>';
                  $output .= '</div>';
                $output .= '</div>';
        }
