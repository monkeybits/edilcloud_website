...
if( ! function_exists( 'sc_pix_button' ) )
{
	function sc_pix_button( $attr, $content = null )
	{
        ...
        $output .= '<a href="'.$btn_link.'" class="btn '.$btn_mb.' '. $classes .' btn-'. $btn_size .'" '.$target_out.' '.$btn_inline_style.' '.$anim_type.' '.$anim_delay.' '.$popup_data.'>';
            if (  !empty( $btn_icon ) && empty($btn_icon_position) ) { $output .= '<i class="'.$btn_title_bold.' '. $btn_icon .' '.$icon_classes.' '.$icon_inner_class.' mr-1"></i> '; }
            $output .= '<span class="'.$btn_title_bold.' '.$inner_class.'" '.$btn_inline_style.'>'. $btn_text .'</span>';
