...
if( ! function_exists( 'sc_pix_reviews_slider' ) )
{
	function sc_pix_reviews_slider( $attr, $content = null )
	{
		...
		if(!empty($slides_arr)){
			// $output  .= '<div class="'.$slider_main_classes.' '.$slider_style.' pix-overflow-y-visible2 pix-slider-dots '.$dots_style.' pix-overflow-all-visible overflow-hidden-clip2 '.$slider_size.'" '.$fdata.' >';
			...
			foreach ($slides_arr as $key => $value) {
				...
				if(!empty($value['image'])){
					...
					// $full_image_url = wp_get_attachment_image( $value['image'], 'full', false, $attrs );
					$imgSrc = '';
