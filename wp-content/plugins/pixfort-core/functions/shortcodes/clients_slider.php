	...
	function sc_clients_slider( $attr, $content = null )
	{
	 ...
	 $class_names = join( ' ', $classes );
		...
		if(!empty($clients_arr)){
			...
			$output  .= '<div id="'.$pix_id.'" class="pix-main-slider clients pix-fix-x2 '.$visible_overflow.' '.$slider_style.' '.$slider_effect.' '.$slider_scale.' '.$visible_y.' pix-slider-'.$slider_num.' pix-slider-dots '.$dots_style.' '.$dots_align.'" '.$slider_data.'>';
			foreach ($clients_arr as $key => $value) {
				...
				$output .= '<div class="pix-slider-effects">';
					$output .= '<div class="text-center client">';
						if( !empty($value['link']) ){
							$output .= '<a class="'.$class_names.' d-inline-block pix-p-20  '.$style.'" target="_blank" href="'. $value['link'] .'" title="'. $value['title'] .'">';
						}else{
						...
						if( !empty($value['image']) ) {
			                ...
			                $output .= '<span class="d-inline-block">';
								if(empty($imgSrcset)){
									$output .= '<img src="'.$imgSrc.'" loading="lazy" class="scale-with-grid animate-in" alt="'.$title.'" data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">';
								}else{
									$output .= '<img src="'.$imgSrc.'" srcset="'.$imgSrcset.'" sizes="'.$imgSizes.'" loading="lazy" class="scale-with-grid animate-in" alt="'.$title.'" data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">';
								}
							$output .= '</span>';
			            }
						if( !empty($value['link']) ){
