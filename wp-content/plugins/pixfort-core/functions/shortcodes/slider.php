	...
	function sc_pix_slider( $attr, $content = null ){
		...
		if(!empty($slides_arr)){
			...
			$output .= '<div class="pix-slider-div '.$class_names.' '.$out_class.'" '.$anim_type.' '.$anim_delay.'>';
				$output .= '<div id="'.$pix_id.'" class="pix-slider '.$top_class.' bg-'.$overlay_color.' pix-slider-full no-dots" '.$el_style.'>';
							                ...
							                <div class="container">
							                    <div class="row row-eq-height align-items-center pix-py-200" >
							                        <div class="col-12 '.$columns_align_size.' pix-py-50 pix-px-40 d-md-flex">
							                            ...
							                            if(!empty($value['title'])) $output .= '<'.$title_tag.' class="pix-sliding-headline '.$t_class_names.' animate-in" data-anim-type="fade-in-up" data-anim-delay="1000" style="'.$t_custom_color. $t_size_style.'">'.$value['title'].'</'.$title_tag.'>';
							                            if(!empty($value['text'])) {
														...
														if(!empty($value['btn_text'])){
															...
															if(!empty($value['btn_popup_id'])) $attr['btn_popup_id'] = $value['btn_popup_id'];
															$attr['css'] = '';
