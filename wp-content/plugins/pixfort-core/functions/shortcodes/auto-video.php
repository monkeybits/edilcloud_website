...
if( ! function_exists( 'sc_pix_auto_video' ) ){
	...
	function sc_pix_auto_video( $attr, $content = null ){
        ...
        if(!empty($mp4_video)){
			// $mp4_video_meta = wp_get_attachment_metadata( $mp4_video);
			// if(!empty($box_media_meta['mime_type'])&&$box_media_meta['mime_type']=='video/mp4'){
			// 	$box_video_url = wp_get_attachment_url( $box_img_obj);
				$poster_tag = '';
				if(!empty($poster)){
				...
				// global $wp_embed;
				// $embed = $wp_embed->run_shortcode( '[embed]' . $mp4_video . '[/embed]' );
	            $classes = array();
	            $anim_type = '';
	            $anim_delay = '';
				...
				if(!empty($img_div)){
					...
					$output .= '<div class="d-inline-block '.$pix_scale_in.'">';
				}
	            if($link){
	                $ntab = '';
	                if(!empty($target)){
					...
					if(!empty($pix_tilt)){
						$output .= '<div  '.$inline_style.' class=" '.$pix_tilt_size.'">';
					}
					...
					$output .= '<video class="pix-video-bg-element d-block '.$rounded_img.' bg-video" '.$width_attr.' '.$height_attr.' preload="none" '.$loop.' '.$poster_tag.' muted playsinline>
					...
					if(!empty($pix_tilt)){
						$output .= '<div  '.$inline_style.' class="d-inline-block '.$pix_tilt_size.'">';
					}
					...
					// )); autoplay="autoplay"
					$output .= '<video class="pix-video-bg-element d-block pix-bg-image2 '.$rounded_img.' bg-video" '.$width_attr.' '.$height_attr.' preload="none" '.$loop.' '.$poster_tag.' muted playsinline>
							<source src="' . $mp4_video . '" type="video/mp4" />
							</video>';
