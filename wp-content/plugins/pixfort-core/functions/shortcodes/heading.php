...
if( ! function_exists( 'sc_heading' ) )
{
	function sc_heading( $attr, $content = null )
	{
		...
		// style="background-image:url('images/event/bg-2.jpg')"
        ...
        $output = '<div '.$heading_id.' class="'.$position.' '.$css_class.'">';
			if(!empty($max_width)) {
				$custom_style = 'style="max-width:'.$max_width.';"';
				$output .= '<div class="d-inline-block" '.$custom_style.'>';
			}
            $output .= '<div><div class="slide-in-container"><'.$title_tag.' class="'. $class_names .' mb-1" style="'.$t_custom_color. $t_size_style.'" data-anim-type="'. $animation .'" data-anim-delay="'.$delay.'">'. $title .'</'.$title_tag.'></div></div>';

			// if( $style == 'line' ) { $output .= '<div><div class="pix-heading-line bg-gradient-primary"></div></div>'; }

            if(!empty($content)){ $output .= '<div class="slide-in-container w-100"><p class="mb-0 '.$c_color.' '.$content_size.' animate-in" data-anim-type="'. $animation .'" data-anim-delay="'.$delay.'" style="'.$c_custom_color.'">'. do_shortcode( $content ) .'</p></div>'; }
