    ...
    function sc_clients( $attr, $content = null )
	{
      ...
      'delay_items' 		=> '',
       ...
       $class_names = join( ' ', $classes );
        ...
        $output  .= '<div class="clients row '. $css_class .'">';
        if($in_row!=5){
        ...
        foreach ($clients_arr as $key => $value) {
            ...
            $output .= '<div class="col-122 col-md col-md2 -'.$in_row.' '.$class_names.' text-center client2 d-inline-block '.$style.'">';

            if( !empty($value['link']) ){
                $output .= '<a target="_blank" href="'. $value['link'] .'" class="client py-3 d-inline-block w-100" title="'. $value['title'] .'">';
            }else{
                $title = '';
                if(!empty($value['title'])) {
                    $title = $value['title'];
                }
                $output .= '<div class="client2 py-3 d-inline-block w-100" title="'. $title .'">';
            }
            if( !empty($value['image']) ) {
                ...
                if(empty($imgSrcset)){
                    $output .= '<img src="'.$imgSrc.'" loading="lazy" class="animate-in" alt="'.$title.'" data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">';    
                }else{
                    $output .= '<img src="'.$imgSrc.'" loading="lazy" srcset="'.$imgSrcset.'" sizes="'.$imgSizes.'" class="animate-in" alt="'.$title.'" data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">';
                }
