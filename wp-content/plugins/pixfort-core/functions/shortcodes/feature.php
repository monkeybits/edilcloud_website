...
if( ! function_exists( 'sc_pix_feature' ) )
{
	function sc_pix_feature( $attr, $content = null )
	{
		...
		if(empty($el_id)){
			$el_id = 'duo-icon-'.rand(1,999999999999);
		}
        ...
        if($icon_position=="left"){
            $output .= '<div id="'.$el_id.'" class="media '.$css_class.'">';
                if($media_type == "icon") {
					if(!empty($has_icon_bg)){
						...
						$output .= '<div class="mr-3 feature_img '.$anim_class.'" '.$anim_type.' '.$anim_delay_icon.' style="position:relative;text-align:center;"><i style="display:inline-block;font-size:'.$icon_size.'px;line-height:'.$icon_size.'px;'.$i_custom_color.'" class="'.$i_color.' align-middle '. $icon .'"></i></div>';
					}
				}
                if($media_type == "image") {
				...
				if($media_type == "duo_icon"){
					if(!empty($pix_duo_icon)){
						if(!empty($has_icon_bg)){
							$output .= '<div class="rounded-circle mr-3 d-inline-block '.$i_bg_color.' '.$anim_class.'" '.$anim_type.' '.$anim_delay_icon.' style="'.$i_bg_custom_color.' width:'.$icon_size_div.'px;height:'.$icon_size_div.'px;position:relative;line-height:'.$icon_size_div.'px;text-align:center;">';
            ...
            $output .= '<div id="'.$el_id.'" class="'.$classes.' '.$css_class.'">';
				if($media_type == "duo_icon"){
					if(!empty($pix_duo_icon)){
						if(!empty($has_icon_bg)){
							$output .= '<div class="rounded-circle d-inline-block '.$i_bg_color.' '.$anim_class.'" '.$anim_type.' '.$anim_delay_icon.' style="'.$i_bg_custom_color.' width:'.$icon_size_div.'px;height:'.$icon_size_div.'px;position:relative;line-height:'.$icon_size_div.'px;text-align:center;">';
								$output .= '<div class="'.$i_color.'" style="display:inline-block;width:'.$icon_size.'px;height:'.$icon_size.'px;'.$i_custom_color.'font-size:'.$icon_size.'px;">';
									$output .= pix_load_inline_svg(PIX_CORE_PLUGIN_DIR.'/functions/images/icons/'.$pix_duo_icon.'.svg');
								$output .= '</div>';
							$output .= '</div>';
