...
/* ---------------------------------------------------------------------------
 * Image [pix_img_carousel] [/pix_img_carousel]
* --------------------------------------------------------------------------- */
if( ! function_exists( 'sc_pix_img_carousel' ) ){

	function sc_pix_img_carousel( $attr, $content = null ){
			...
			foreach ($slides_arr as $key => $value) {
				...
				if(!empty($value['image'])){
		            ...
		            if(!empty($value['link'])){
		                ...
		                $output .= '<img class="card-img '.$rounded_img.' h-100" sizes="'.$imgSizes.'" srcset="'.$imgSrcset.'" src="'.$imgSrc.'" alt="'. $value['alt'] .'" '.$inline_style.'/>';
		                $output .= '</a>';
						if(!empty($pix_tilt)){
							$output .= '</div>';
						}
		                ...
		                $output .= '<img class="card-img '.$rounded_img.' h-100" srcset="'.$imgSrcset.'" sizes="'.$imgSizes.'" src="'.$imgSrc.'" alt="'. $value['alt'] .'" '.$inline_style.'/>';
		                $output .= '</div>';
...
add_shortcode( 'pix_img_carousel', 'sc_pix_img_carousel' );
