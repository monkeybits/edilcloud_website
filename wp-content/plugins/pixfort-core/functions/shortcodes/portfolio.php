...
function pix_content_portfolio( $query = false, $attr = false  ){
    ...
    $output .= '<div class="pix-portfolio">';
		if($filters){
			$output .= pix_portfolio_nav($filters_align, $filter_light);
		}
...
function pix_portfolio_nav($filters_align, $filter_light){
	...
	if ( $query && $query->have_posts() ){
		...
		if($is_col) $output .='<div class="col-12 pb-4 grid-item '. $item_class .'" data-category="transition">';
			$output .='<div class="card w-100">';
				...
				$full_image_url = wp_get_attachment_image( get_post_thumbnail_id(), 'pix-portfolio-small', false, $attrs );
	...
	if ( $query && $query->have_posts() ){
		...
		if($is_col) $output .='<div class="col-12 pb-4 grid-item '. $item_class .'" data-category="transition">';
			$output .='<a href="'. get_permalink() .'" class=" fly-sm">';
				$output .='<div class="card bg-white '.$rounded_img.' overflow-hidden fly-sm shadow shadow-hover pix-mb-302 animate-in" data-anim-type="fade-in">';
				  $output .= $img_src;
	...
	if ( $query && $query->have_posts() ){
		...
		if( is_array( $terms ) ){
			foreach( $terms as $term ){
				...
				$cats_str .= '<a class="mb-1 d-inline-block" href="'. site_url() .'/?portfolio-types='. $term->slug .'">';
				$cats_str .= sc_pix_badge($badge_attrs);
				$cats_str .= '</a>';
			}
			$categories = substr( $categories , 0, -2 );
		...
		if($is_col) $output .='<div class="col-12 pb-4 grid-item '. $item_class .'" data-category="transition">';
			$output .='<div class="card bg-white '.$rounded_img.' overflow-hidden fly-sm shadow w-100 animate-in" data-anim-type="fade-in">';
				$attrs = array(
