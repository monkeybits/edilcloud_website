...
if(function_exists('vc_shortcode_custom_css_class')){
    $css_class = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, vc_shortcode_custom_css_class( $css, ' ' ) );
		...
		$output = '';
		$output .= '<div class="card pix-hover-item '.$class_names.' '.$position.' rounded-lg bg-'.$overlay_color.' d-flex align-items-end pix-team-card big-card hover-bg" '.$custom_style.' >';
			if(!empty($image)){
        		...
        		// $output .= '<img src="'.PIX_IMG_PLACEHOLDER .'" data-src="'.$imgSrc.'" data-srcset="'.$imgSrcset.'" class="pix-lazy card-img '.$overlay_opacity.'" alt="...">';
				if(empty($imgSrcset)){
        	...
        	$output .= '<div class="card-img-overlay d-flex flex-column justify-content-end">';
            	$output .= '<'.$name_tag.' class="card-title mb-0 '.$name_classes.' animate-in" '.$name_style.' data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">'.$name.'</'.$name_tag.'>';
				$delay+=100;
            	$output .= '<'.$title_tag.' class="'.$title_classes.' mb-2 animate-in" '.$title_style.' data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">'.$title.'</'.$title_tag.'>';
				$delay+=100;
				if(!empty($description)) {
					$output .= '<p class="p-0 animate-in '.$d_classes.'" '.$d_custom_color.' data-anim-type="'.$animation.'" data-anim-delay="'.$delay.'">'. do_shortcode( $description ) .'</p>';
					$delay+=100;
				}
