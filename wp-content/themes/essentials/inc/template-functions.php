...
add_filter( 'comment_form_submit_button', function( $submit_button, $args ) {
	...
	// $button = '<div class="col-md-12"><input name="%1$s" type="submit" id="%2$s" class="%3$s" value="%4$s" /></div>';
...
if ( ! function_exists( 'essentials_search_overlay' ) ) {
	function essentials_search_overlay() {
		...
		if(!empty(pix_get_option('search-style'))){
			...
			$output .= '<svg class="shape-overlays" viewBox="0 0 100 100" preserveAspectRatio="none">';
				$output .= '<defs>';
				$output .= $colors;
...
*/
// function pix_show_option( $opt_name_val, $default = null ){
//  	if( function_exists( 'pix_plugin_show_option' ) ){
// 		return pix_plugin_show_option($opt_name_val);
// 	}else{
// 		return $default;
// 	}
// }
...
if ( ! function_exists( 'pix_get_breadcrumb' ) ) {
	function pix_get_breadcrumb($theme = 'dark', $align = 'justify-content-start') {
	    ...
	    if (!is_front_page()) {
	        ...
	        <nav class="text-center" aria-label="breadcrumb">
	        	<ol class="breadcrumb px-0 <?php echo esc_attr( $align ); ?>">
	        		<li class="breadcrumb-item animate-in" data-anim-type="fade-in-left" data-anim-delay="500"><a class="<?php echo esc_attr( $link_classes ); ?>" href="<?php echo esc_url(home_url('/')); ?>"><?php echo esc_attr_e( 'Home', 'essentials' ); ?></a></li>
			<?php
				...
				if(get_the_category()){
		            ...
		            if($cat->cat_name){
						...
						<li class="breadcrumb-item animate-in" data-anim-type="fade-in-left" data-anim-delay="500">
							...
							<a class="<?php echo esc_attr( $link_classes ); ?>" href="<?php echo esc_url( get_category_link($cat) ); ?>"><?php echo esc_attr( $cat->cat_name ); ?></a>
						</li>
						<?php
					}
				}
		...
		global $woocommerce;
		if( $woocommerce && (is_shop() || is_product_category()) ){
			?>
				...
				<div class="row">
					...
					<div class="col-12 col-sm-6 pix-py-20">
						...
						<p class="text-body-default pix-popup-product-desc"><?php echo  do_shortcode( $product->get_short_description() ); ?></p>
					 	<?php echo wc_get_template_html( 'single-product/add-to-cart/simple.php' ); ?>
					</div>
				</div>
			</div>
			<?php
...
function pix_ajax_searcht() {
	if ( !wp_verify_nonce( $_REQUEST['nonce'], "search_nonce")) {
		exit("Verification error, please try again!");
	}
